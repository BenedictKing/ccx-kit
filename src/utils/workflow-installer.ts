...
async function installWorkflowWithDependencies(
  ...
  const commandsDir = join(CLAUDE_DIR, 'commands', 'ccx-kit')
  ...
  for (const commandFile of config.commands) {
    ...
    if (existsSync(commandSource)) {
      try {
        ...
        console.log(ansis.gray(`  ✔ ${i18n.t('workflow:installedCommand')}: ccx-kit/${destFileName}`))
      }
      catch (error) {
  ...
  if (config.autoInstallAgents && config.agents.length > 0) {
    const agentsCategoryDir = join(CLAUDE_DIR, 'agents', 'ccx-kit', config.category)
    ...
    for (const agent of config.agents) {
      ...
      if (existsSync(agentSource)) {
        try {
          ...
          console.log(ansis.gray(`  ✔ ${i18n.t('workflow:installedAgent')}: ccx-kit/${config.category}/${agent.filename}`))
        }
        catch (error) {
