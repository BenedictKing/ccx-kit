...
describe('error-handler utilities', () => {
  ...
  describe('handleExitPromptError', () => {
    it('should handle ExitPromptError and exit gracefully', async () => {
      ...
      vi.mocked(readAppConfig).mockReturnValue({ preferredLang: 'zh-CN', version: '1.0.0', lastUpdated: '2024-01-01', codeToolType: 'claude-code' })
  ...
  describe('handleGeneralError', () => {
    it('should handle Error objects with stack trace', async () => {
      const { readAppConfig } = await import('../../../src/utils/app-config')
      vi.mocked(readAppConfig).mockReturnValue({ preferredLang: 'en', version: '1.0.0', lastUpdated: '2024-01-01', codeToolType: 'claude-code' })
