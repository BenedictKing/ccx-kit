...
# examples:
#   - /ccx-kit-update-docs                 # Check and update all documentation files
#   - /ccx-kit-update-docs --check-only    # Only check for inconsistencies without making updates (dry run)
---

# CCX-Kit Update Docs - Documentation Synchronization
...
```bash
/ccx-kit-update-docs [--check-only]
```
...
case "$ARGUMENTS" in
  ...
  *)
    ...
    echo "Usage: /ccx-kit-update-docs [--check-only]"
    exit 1
    ;;
esac
```
