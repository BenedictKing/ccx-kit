---
description: Manage Git worktrees in project-level ../.ccx-kit/project-name/ directory with smart defaults, IDE integration and content migration
...
Manage Git worktrees with smart defaults, IDE integration and content migration in structured `../.ccx-kit/project-name/` paths.
...
| Option             | Description                                            |
| ------------------ | ------------------------------------------------------ |
| `add [<path>]`     | Add new worktree in `../.ccx-kit/project-name/<path>`      |
...
2. **Smart Path Management**
   ...
   - Create worktrees in structured `../.ccx-kit/project-name/<path>` directory
...
WORKTREE_BASE="$MAIN_REPO_PATH/../.ccx-kit/$PROJECT_NAME"

# Always use absolute path to prevent nesting issues
ABSOLUTE_WORKTREE_PATH="$WORKTREE_BASE/<path>"
```

**Critical Fix**: Always use absolute paths when creating worktrees from within existing worktrees to prevent path nesting issues like `../.ccx-kit/project/.ccx-kit/project/path`.
...
6. **Safety Features**
   ...
   - **Absolute path enforcement**: Prevent nested `.ccx-kit` directories when in worktree
...
```
✅ Worktree created at ../.ccx-kit/project-name/feature-ui
✅ Copied .env
✅ Copied .env.local
📋 Copied 2 environment file(s) from .gitignore
🖥️ Open ../.ccx-kit/project-name/feature-ui in IDE? [y/n]: y
🚀 Opening ../.ccx-kit/project-name/feature-ui in VS Code...
```
