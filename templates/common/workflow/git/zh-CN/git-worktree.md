---
description: 管理 Git worktree，在项目平级的 ../.ccx-kit/项目名/ 目录下创建，支持智能默认、IDE 集成和内容迁移
...
管理 Git worktree，支持智能默认、IDE 集成和内容迁移，使用结构化的 `../.ccx-kit/项目名/` 路径。
...
| 选项               | 说明                                         |
| ------------------ | -------------------------------------------- |
| `add [<path>]`     | 在 `../.ccx-kit/项目名/<path>` 添加新的 worktree |
...
2. **智能路径管理**
   ...
   - 在结构化的 `../.ccx-kit/项目名/<path>` 目录创建 worktree
...
**关键修复**: 在现有 worktree 内创建新 worktree 时，始终使用绝对路径以防止出现类似 `../.ccx-kit/project/.ccx-kit/project/path` 的路径嵌套问题。
...
6. **安全特性**
   - **路径冲突防护**: 创建前检查目录是否已存在
   - **分支检出验证**: 确保分支未被其他地方使用
   - **绝对路径强制**: 防止在 worktree 内创建嵌套的 `.ccx-kit` 目录
...
```
✅ Worktree created at ../.ccx-kit/项目名/feature-ui
✅ 已复制 .env
✅ 已复制 .env.local
📋 已从 .gitignore 复制 2 个环境文件
🖥️ 是否在 IDE 中打开 ../.ccx-kit/项目名/feature-ui？[y/n]: y
🚀 正在用 VS Code 打开 ../.ccx-kit/项目名/feature-ui...
```
