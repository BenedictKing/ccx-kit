...
```bash
/ccx-kit:workflow <TASK_DESCRIPTION>
```
...
- Store plan summary (with context and plan) in project root directory `.ccx-kit/plan/current/task-name.md`
...
- Naming task plan documents during archiving (when moving from `.ccx-kit/plan/current/` to `.ccx-kit/plan/history/`)
...
- Store execution plan in project root directory `.ccx-kit/plan/current/task-name.md`
...
- After task completion, move plan file from `.ccx-kit/plan/current/` to `.ccx-kit/plan/history/` for archiving
