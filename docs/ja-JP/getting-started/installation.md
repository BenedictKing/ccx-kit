...
> 💡 **ヒント**: ZCFはインストール不要で、`npx ccx-kit`を直接実行できます。このガイドは主に環境設定と使用フローについて説明しています。
...
> 💡 **ヒント**: ZCFはインストール不要で、`npx ccx-kit`コマンドを直接実行できます。
...
> 📖 **完全なパラメータリスト**: 詳細なパラメータ説明については、[CLIコマンド - ccx-kit init](../cli/init.md)の章を参照してください。
...
# 方法2: 対話式メニュー経由
npx ccx-kit → S（ツール切り替え）を選択 → 1（完全初期化）を選択
```
...
> ⚠️ **注意**: WindowsでMCP接続の問題が発生した場合、`npx ccx-kit`を実行すると設定形式が自動的に修正されます。
...
> 🔧 **トラブルシューティング**: サービスが接続されていない場合、`npx ccx-kit` → `4`を実行してMCPサービスを再設定してください。
...
> 📖 **注意**: `ccx-kit update`は、デフォルトで既存のAPI設定とMCP設定を保持し、ワークフローテンプレートとドキュメントのみを更新します。
...
ファイルが存在しない場合、`npx ccx-kit update`を実行して再インポートしてください。

### Q: MCPサービスの接続に失敗しましたか？

**A**: 
1. MCPサービス設定を確認: `npx ccx-kit` → `4`
...
**A**: 設定切り替えコマンドを使用：
```bash
npx ccx-kit config-switch --list  # すべての設定を一覧表示
npx ccx-kit cs provider-name      # 指定された設定に切り替え
```

### Q: 設定ファイルはどこに保存されますか？

**A**: 
- **ZCF設定**: `~/.ccx-kit/config.toml`
