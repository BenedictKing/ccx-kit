...
`ccx-kit ccx` は Claude Code Router（CCR）の完全な管理メニューを提供し、インストール、設定、サービス制御、Web UI アクセスなどの機能を含みます。
...
`ccx-kit ccx` を実行すると、以下のメニューが表示されます：
...
**API キー**：
- UI 起動時に CCR API キーが表示される（デフォルト：`sk-ccx-kit`）
...
| `APIKEY` | string | CCR API キー | `sk-ccx-kit` |
...
3. API キー `sk-ccx-kit` でログイン（または設定の `APIKEY` を確認）
