...
`ccx-kit init`（省略形 `ccx-kit i`）は ZCF のコアコマンドで、Claude Code または Codex 環境を完全に初期化するために使用されます。必要なツールのインストール、API の設定、MCP サービスの設定、ワークフローと出力スタイルのインポートなどを自動的に実行します。

## 機能概要

`ccx-kit init` コマンドは以下の操作を実行します：
...
# 設定処理方法
npx ccx-kit i -s --config-action backup  # バックアップ後に上書き（デフォルト）
npx ccx-kit i -s --config-action merge   # 設定をマージ
npx ccx-kit i -s --config-action new     # 新しい設定を作成
npx ccx-kit i -s --config-action docs-only  # ドキュメントのみ更新
npx ccx-kit i -s --config-action skip    # 設定をスキップ

# CCometixLine インストールを制御
npx ccx-kit i -s -x true   # インストール（デフォルト）
npx ccx-kit i -s -x false  # インストールしない
```
