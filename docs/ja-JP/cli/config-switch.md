...
`ccx-kit config-switch` は複数の API 設定間を素早く切り替えるために使用されます。異なるプロジェクトで異なる API プロバイダーを使用するユーザーに適しています。

> **別名**：`ccx-kit cs` という短縮形を使用できます。すべての例は `npx ccx-kit cs --list` などの形式に書き換えることができます。
...
npx ccx-kit cs --list -T cc      # Claude Code 設定を一覧表示
npx ccx-kit cs --list -T cx      # Codex 設定を一覧表示
npx ccx-kit cs provider1 -T cx   # Codex 設定を切替
```
...
3. **カスタム設定**：`ccx-kit init` で作成された複数の API 設定
...
2. **カスタムプロバイダー**：`ccx-kit init` で設定されたプロバイダー（302.AI、GLM など）
...
```bash
# Claude Code 設定
npx ccx-kit cs --list --code-type claude-code

# Codex 設定
npx ccx-kit cs --list --code-type codex
```
...
# Codex プロバイダーを切替
npx ccx-kit cs glm-provider --code-type codex
```
...
```bash
# Codex プロバイダーを一覧表示
npx ccx-kit cs --code-type codex --list
