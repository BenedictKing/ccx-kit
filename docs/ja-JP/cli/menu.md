...
`npx ccx-kit` を実行すると、対話式メニューシステムに入ります。メニューは ZCF のコアインターフェースで、視覚的な操作オプションを提供し、複雑なコマンドパラメータを覚える必要がありません。
...
| オプション | 機能 | 対応コマンド | 説明 |
|------|------|---------|------|
| `1` | 完全初期化 | `ccx-kit init` | Claude Code 環境を完全に初期化 |
| `2` | ワークフローのインポート/更新 | `ccx-kit update` | ワークフローテンプレートとプロンプトを更新 |
...
| `R` | CCR 管理メニュー | `ccx-kit ccx` | Claude Code Router 管理 |
| `U` | Claude Code 使用分析 | `ccx-kit ccu` | API 使用統計を表示 |
| `L` | CCometixLine 管理 | - | ステータスバーツール管理 |
| `0` | スクリプト言語を切替 | - | CLI インターフェース言語を切替（zh-CN/en） |
| `S` | コードツールを切替 | - | Claude Code ↔ Codex の間で切替 |
| `-` | 現在のツール設定をアンインストール | `ccx-kit uninstall` | Claude Code 設定をアンインストール |
| `+` | 更新を確認 | `ccx-kit check-updates` | ツールバージョンを確認して更新 |
| `Q` | 終了 | - | メニューを終了 |
...
`npx ccx-kit init` を実行するのと同じで、以下を案内します：
...
`npx ccx-kit update` を実行するのと同じで、以下を実行します：
...
# 2. 手動で切替
npx ccx-kit init -T codex  # Codex に切替
npx ccx-kit init -T claude-code  # Claude Code に切替
```

## 関連リソース

- [ccx-kit init](init.md) - 完全初期化コマンドの詳細
- [ccx-kit update](update.md) - 更新コマンドの詳細
