...
1. **グローバル設定**（`~/.ccx-kit/config.toml`）- ZCF 自体の設定
...
- **設定作成**：`ccx-kit init` コマンドを使用して初期化時に複数の API プロバイダーを設定できます。
- **設定切替**：`ccx-kit config-switch` コマンドを使用して、異なる環境、プロジェクト、プロバイダー間を素早く切り替えます。
...
| **ZCF グローバル** | `~/.ccx-kit/backup/` | `config.toml.{timestamp}.bak` |
...
1. **初期化**：初回設定または再初期化
2. **設定更新**：`ccx-kit update` によるワークフローまたはテンプレートの更新
...
2. **復元する**：手動でバックアップファイルを元の場所にコピーするか、`ccx-kit init` のマージ機能を使用する
