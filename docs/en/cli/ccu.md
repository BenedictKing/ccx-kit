...
`ccx-kit ccu` (Claude Code Usage) is used to view and analyze Claude Code usage statistics, helping you understand AI assistant usage and costs.
...
npx ccx-kit ccu --period daily
npx ccx-kit ccu --period weekly
npx ccx-kit ccu --period monthly

# JSON format output (for script processing)
npx ccx-kit ccu --json
...
```bash
npx ccx-kit ccu --json --period weekly
```
...
```bash
npx ccx-kit ccu --csv --period monthly > usage.csv
```
...
npx ccx-kit ccu --period weekly --json > weekly-usage.json
```
...
npx ccx-kit ccu --period monthly --csv > monthly-usage.csv
...
0 23 * * * cd /path/to/project && npx ccx-kit ccu --json --period daily >> usage.log
```
...
1. Install CCometixLine: `npx ccx-kit` → Select corresponding option
