...
Running `npx ccx-kit` enters the interactive menu system. The menu is ZCF's core interactive interface, providing visual operation options without needing to remember complex command parameters.
...
|------|------|---------|------|
| `1` | Complete Initialization | `ccx-kit init` | Complete Claude Code environment initialization |
| `2` | Import/Update Workflows | `ccx-kit update` | Update workflow templates and prompts |
...
| `R` | CCR Management Menu | `ccx-kit ccx` | Claude Code Router management |
| `U` | Claude Code Usage Analysis | `ccx-kit ccu` | View API usage statistics |
...
| `-` | Uninstall Current Tool Configuration | `ccx-kit uninstall` | Uninstall Claude Code configuration |
| `+` | Check Updates | `ccx-kit check-updates` | Check tool versions and update |
| `Q` | Exit | - | Exit menu |
...
Equivalent to running `npx ccx-kit init`, will guide you through:
...
Equivalent to running `npx ccx-kit update`, will:
...
cat ~/.ccx-kit/config.toml | grep codeToolType

# 2. Manually switch
npx ccx-kit init -T codex  # Switch to Codex
npx ccx-kit init -T claude-code  # Switch to Claude Code
```

## Related Resources

- [ccx-kit init](init.md) - Detailed complete initialization command
- [ccx-kit update](update.md) - Detailed update command
