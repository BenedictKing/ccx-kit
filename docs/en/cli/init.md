...
`ccx-kit init` (abbreviation `ccx-kit i`) is ZCF's core command for complete initialization of Claude Code or Codex environment. It automatically installs necessary tools, configures API, sets up MCP services, imports workflows and output styles, etc.

## Feature Overview

The `ccx-kit init` command performs the following operations:
...
# Use GLM provider
npx ccx-kit i -s -p glm -k "sk-xxx"

# Use MiniMax provider
npx ccx-kit i -s -p minimax -k "sk-xxx"

# Use Kimi provider
npx ccx-kit i -s -p kimi -k "sk-xxx"

# Use custom provider (requires URL)
npx ccx-kit i -s -p custom -k "sk-xxx" -u "https://api.example.com"
```
...
```bash
# Use API Key
npx ccx-kit i -s -t api_key -k "sk-ant-xxx"

# Use Auth Token (official login)
npx ccx-kit i -s -t auth_token -k "your-auth-token"

# Use CCR proxy
npx ccx-kit i -s -t ccr_proxy

# Skip API configuration
npx ccx-kit i -s -t skip

# Configure custom models
npx ccx-kit i -s -t api_key -k "sk-xxx" -M "claude-sonnet-4-5" -H "claude-haiku-4-5" -S "claude-sonnet-4-5" -O "claude-opus-4-5"
```
...
# Use JSON file
npx ccx-kit i -s --api-configs-file ./api-configs.json
```
...
npx ccx-kit i -s -m all

# Install specific services (comma-separated)
npx ccx-kit i -s -m context7,open-websearch,spec-workflow

# Skip MCP service installation
npx ccx-kit i -s -m skip
```
...
npx ccx-kit i -s -w all

# Install specific workflows
npx ccx-kit i -s -w sixStepsWorkflow,gitWorkflow

# Skip workflow installation
npx ccx-kit i -s -w skip
```
...
npx ccx-kit i -s -o all

# Install specific styles
npx ccx-kit i -s -o engineer-professional,nekomata-engineer

# Set default output style
npx ccx-kit i -s -o all -d engineer-professional

# Skip output style installation
npx ccx-kit i -s -o skip
```
...
npx ccx-kit i -s --config-action backup  # Backup then overwrite (default)
npx ccx-kit i -s --config-action merge   # Merge configuration
npx ccx-kit i -s --config-action new     # Create new configuration
npx ccx-kit i -s --config-action docs-only  # Only update documents
npx ccx-kit i -s --config-action skip    # Skip configuration

# Control CCometixLine installation
npx ccx-kit i -s -x true   # Install (default)
npx ccx-kit i -s -x false  # Don't install
```
...
```bash
npx ccx-kit i -s -p 302ai -k "sk-xxx" -g zh-CN
```
...
npx ccx-kit i -s --api-configs-file ./api-configs.json -g zh-CN
```

### Scenario 5: Only Update Documents and Templates

```bash
npx ccx-kit i -s --config-action docs-only -g zh-CN
```

### Scenario 6: Use CCR Proxy

```bash
npx ccx-kit i -s -t ccr_proxy -g zh-CN -m all -w all
```
...
The execution flow of `ccx-kit init` is as follows:
