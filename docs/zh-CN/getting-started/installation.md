...
> 💡 **提示**：ZCF 无需安装，直接使用 `npx ccx-kit` 即可运行。本指南主要介绍环境配置和使用流程。
...
> 💡 **提示**：ZCF 无需安装，直接使用 `npx ccx-kit` 命令即可运行。
...
> 📖 **完整参数列表**：详细参数说明请参考 [CLI 命令 - ccx-kit init](../cli/init.md) 章节。
...
# 方式 2：通过交互式菜单
npx ccx-kit → 选择 S（切换工具）→ 选择 1（完整初始化）
```
...
> ⚠️ **注意**：如果在 Windows 上遇到 MCP 连接问题，运行 `npx ccx-kit` 会自动修复配置格式。
...
> 🔧 **故障排除**：如果服务未连接，运行 `npx ccx-kit` → `4` 重新配置 MCP 服务。
...
> 📖 **说明**：`ccx-kit update` 默认会保留您现有的 API 配置和 MCP 设置，只更新工作流模板和文档。
...
如果文件不存在，运行 `npx ccx-kit update` 重新导入。

### Q: MCP 服务连接失败？

**A**: 
1. 检查 MCP 服务配置：`npx ccx-kit` → `4`
...
**A**: 使用配置切换命令：
```bash
npx ccx-kit config-switch --list  # 列出所有配置
npx ccx-kit cs provider-name      # 切换到指定配置
```

### Q: 配置文件保存在哪里？

**A**: 
- **ZCF 配置**：`~/.ccx-kit/config.toml`
