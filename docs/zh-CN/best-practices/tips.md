...
**技巧**：习惯从 `npx ccx-kit` 开始，所有功能都有编号提示，避免记忆命令细节。
...
```bash
#!/bin/bash
# deploy-ccx-kit.sh - 自动化部署 CCX-Kit 配置
...
```bash
# 1. 仅安装需要的 MCP 服务
npx ccx-kit init -s -m context7,open-websearch  # 仅安装必需服务
...
# 团队成员使用相同配置
npx ccx-kit init -s --api-configs-file team-config.json -k "个人API密钥"
```
