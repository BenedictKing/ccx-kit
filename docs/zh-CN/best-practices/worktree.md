...
Git Worktree 是 Git 的一个强大功能，允许您在同一仓库中创建多个工作区（副本），每个工作区可以切换不同的分支，互不干扰，共享同一套 Git 历史。CCX-Kit 提供的 `/git-worktree` 智能指令让 Worktree 管理变得简单直观。
...
CCX-Kit 的 `/git-worktree` 指令提供了完整的 Worktree 管理功能：
...
CCX-Kit 的 `/git-worktree` 指令支持自然语言，不需要记住命令细节：
...
CCX-Kit 的 Worktree 默认创建在项目平级的 `../.zcf/项目名/` 目录下，避免混乱：
...
- [CCX-Kit 六阶段工作流](../workflows/zcf-workflow.md) - 配合工作流使用
