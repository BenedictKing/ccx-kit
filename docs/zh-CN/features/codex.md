...
> 📖 **切换提供商**：使用 `npx ccx-kit config-switch -T codex` 在多个提供商之间切换。
...
# 通过菜单配置
npx ccx-kit → 选择 S（切换到 Codex）→ 选择 4（配置 MCP）
```
...
# 通过菜单导入
npx ccx-kit → 选择 S（切换到 Codex）→ 选择 4（导入工作流）
```
...
# 交互式方式
npx ccx-kit → 选择 S（切换到 Codex）→ 选择 1（完整初始化）
```
...
Codex 可以通过 CCX 代理使用第三方模型（如 DeepSeek、GLM 等），实现低成本高效编码。

### 工作原理

Codex 使用 OpenAI Responses API 协议（`wire_api=responses`）与 CCX 通信。CCX 的 Fuzzy Mode 会自动将 Codex 请求的模型名映射为实际提供商模型。

```
Codex → CCX (http://127.0.0.1:3688/v1) → 上游提供商（DeepSeek 等）
```
...
| 配置项 | 值 | 说明 |
|--------|---|------|
| `base_url` | `http://127.0.0.1:3688/v1` | 注意需要 `/v1` 后缀 |
| 认证方式 | `OPENAI_API_KEY` 环境变量 | 使用 CCX 的 `PROXY_ACCESS_KEY`（非 `CCX_API_KEY`） |
| `requires_openai_auth` | `false` | 无需 OpenAI 官方认证 |
| 模型映射 | 自动 | CCX Fuzzy Mode 自动映射 gpt-5.5 → deepseek-v4-pro |

### 配置示例

Codex 的 `~/.codex/config.toml` 中相关配置：
...
```bash
export OPENAI_API_KEY="sk-ccx-kit"  # CCX 的 PROXY_ACCESS_KEY
```

### 模型映射

通过 CCX 的预设渠道，Codex 请求的模型会自动映射：

| Codex 请求模型 | 实际调用模型（以 DeepSeek 为例） |
|---------------|-------------------------------|
| gpt-5.5 / gpt | deepseek-v4-pro |
| gpt-5.5-mini / mini | deepseek-v4-flash |

### 快速配置

使用 ccx-kit 一键配置 Codex + CCX：
...
> 💡 **提示**：如果在 Windows 下 `127.0.0.1` 无法连接，使用 CCX 菜单选项 10（修复连接地址）自动检测可用地址。
