...
1. **全局配置**（`~/.ccx-kit/config.toml`）- ZCF 本身的配置
...
- **创建配置**：在初始化时可以通过 `ccx-kit init` 命令配置多个 API 提供商。
- **切换配置**：使用 `ccx-kit config-switch` 命令在不同环境、项目或提供商之间快速切换。
...
| **ZCF 全局配置** | `~/.ccx-kit/backup/` | `config.toml.{timestamp}.bak` |
...
1. **初始化配置**：首次配置或重新初始化
2. **更新配置**：通过 `ccx-kit update` 更新工作流或模板
