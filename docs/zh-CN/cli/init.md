...
`ccx-kit init`（缩写 `ccx-kit i`）是 ZCF 的核心命令，用于完整初始化 Claude Code 或 Codex 环境。它会自动安装必要的工具、配置 API、设置 MCP 服务、导入工作流和输出风格等。

## 功能概述

`ccx-kit init` 命令会执行以下操作：
...
# 配置处理方式
npx ccx-kit i -s --config-action backup  # 备份后覆盖（默认）
npx ccx-kit i -s --config-action merge   # 合并配置
npx ccx-kit i -s --config-action new     # 创建新配置
npx ccx-kit i -s --config-action docs-only  # 仅更新文档
npx ccx-kit i -s --config-action skip    # 跳过配置

# 控制 CCometixLine 安装
npx ccx-kit i -s -x true   # 安装（默认）
npx ccx-kit i -s -x false  # 不安装
```
