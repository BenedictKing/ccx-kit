...
执行 `npx ccx-kit` 会进入交互式菜单系统。菜单是 ZCF 的核心交互界面，提供可视化的操作选项，无需记忆复杂的命令参数。
...
| 选项 | 功能 | 对应命令 | 说明 |
|------|------|---------|------|
| `1` | 完整初始化 | `ccx-kit init` | 完整初始化 Claude Code 环境 |
| `2` | 导入/更新工作流 | `ccx-kit update` | 更新工作流模板和提示词 |
...
| `R` | CCR 管理菜单 | `ccx-kit ccx` | Claude Code Router 管理 |
| `U` | Claude Code 使用分析 | `ccx-kit ccu` | 查看 API 使用统计 |
| `L` | CCometixLine 管理 | - | 状态栏工具管理 |
| `0` | 切换脚本语言 | - | 切换 CLI 界面语言（zh-CN/en） |
| `S` | 切换代码工具 | - | 在 Claude Code ↔ Codex 之间切换 |
| `-` | 卸载当前工具配置 | `ccx-kit uninstall` | 卸载 Claude Code 配置 |
| `+` | 检查更新 | `ccx-kit check-updates` | 检查工具版本并更新 |
| `Q` | 退出 | - | 退出菜单 |
...
# 2. 手动切换
npx ccx-kit init -T codex  # 切换到 Codex
npx ccx-kit init -T claude-code  # 切换到 Claude Code
```

## 相关资源

- [ccx-kit init](init.md) - 完整初始化命令详解
- [ccx-kit update](update.md) - 更新命令详解
