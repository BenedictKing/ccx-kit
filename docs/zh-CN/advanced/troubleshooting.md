...
**症状**：运行 `npx ccx-kit init` 后无响应或报错
...
# 重新输入正确的密钥
npx ccx-kit init -s -p 302ai -k "正确的密钥"
```
...
- 查看 [GitHub Issues](https://github.com/BenedictKing/ccx-kit/issues) 搜索类似问题
- 在 GitHub Issues 提交问题时附带：
  - `npx ccx-kit --version` 输出
  - 完整的终端日志
  - 操作系统和 Node.js 版本
  - 重现步骤
